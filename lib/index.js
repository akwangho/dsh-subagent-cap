...
function createController(ctx) {
  ...
  //
  // maps parentId -> { inFlight: number, waiters: Waiter[] }
  ...
  async function deliver(parentId) {
    const c = counter(parentId)
    while (c.waiters.length > 0) {
      ...
      try {
        ...
        if (callId !== undefined && callId !== null) heldBy.set(String(callId), parentId)
        w.resolve(w.next()) // admits the held delegate call
      } catch {
        if (c.inFlight > 0) c.inFlight -= 1
      }
    }
  ...
  const offPre = ctx.on('tools/pre-execute', async (exec, next) => {
    ...
    return await new Promise((resolve) => {
      ...
      const waiter = { exec, next, resolve, released: false }
      const onAbort = () => {
        const i = c.waiters.indexOf(waiter)
        if (i >= 0) c.waiters.splice(i, 1)
        resolve({
          kind: 'deny',
          reason: 'Subagent queue wait cancelled.',
        })
      }
